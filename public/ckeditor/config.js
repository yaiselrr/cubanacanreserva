...
CKEDITOR.on('instanceReady', function (e) {
	...
	instance.on('key', function (evt) {
		...
		if (evt.data.keyCode == backSpaceKeyCode || evt.keyCode == deleteKeyCode){
			setTimeout(function () {
				onCKEditorChange(evt.editor);
			},100);
		}
	...
	instance.on('blur', function (evt) {
		onCKEditorLostFocus(evt.editor)
	})
...
function onCKEditorLostFocus(instance){
	instance.updateElement();
	triggerElementLostFocusAndJqueryValidation($(instance.element.$))
}
function triggerElementChangeAndJqueryValidation(element){
	element.trigger('keyup');
}
function triggerElementLostFocusAndJqueryValidation(element){
